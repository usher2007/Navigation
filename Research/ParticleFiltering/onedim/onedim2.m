...
for k = 2:T             % Simulate the system
    x(k) =x(k-1)/2 + 25*x(k-1)/(1 + x(k-1)^2) + 8*cos(1.2*k)+ sqrt(Q)*randn(1);% 计算真实值x
...
tic;
for k = 2:T
    x = x./2 + 25*x./(1 + x.^2) + 8*cos(1.2*k)+sqrt(Q)*randn(1,N);
    h=(x.^2)/20;
...
figure(3)
clf;
plot(1:T,xTrue,'b*-',1:T,xpf,'r^-',1:T,xekf,'g+--'); % 真实值蓝色*表示，粒子滤波值红色三角表示 ，EKF滤波用绿色加号表示
% plot(1:T,xTrue,'b*-');
% hold on
% plot(1:T,xpf,'r^-');
