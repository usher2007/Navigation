function [img] = GetMidianImg(img_src, src_x, src_y, des_x, des_y)
    ...
    sp_des(feature_col,1:(3*img_width+1),1:2) = [[1:(img_width-1)/(3*img_width):img_width]' ones(3*img_width+1,1)*img_height];
    ...
    for i=1:img_width
        intersection_points_aux = [1,sp_mid_des(1,i*3,2);
                                   2,sp_mid_des(2,i*3,2);
                                   3,sp_mid_des(3,i*3,2);
                                   4,sp_mid_des(4,i*3,2)];
        intersection_points_des = [sp_des(1,i*3,2),1;
                                   sp_des(2,i*3,2),2;
                                   sp_des(3,i*3,2),3;
                                   sp_des(4,i*3,2),4];
        ...
        for j=1:img_height
            ...
            des_location_ahead = ceil((des_f(round((j-0.5)*3+1),2)-1)/3.0 * img_height * 3);
            des_location_after = floor((des_f(round((j+0.5)*3+1),2)-1)/3.0 * img_height * 3);
